...
def modelGBM(trainfea, trainlabel, testfea, testlabel, cv=False):
    print("Model GBM...")
    if not cv:
        md = lgb.LGBMClassifier(boosting_type='gbdt', num_leaves=100, max_depth=-1,
        ...
        trainloss = gbm.score(trainfea, trainlabel)
        testpred = gbm.predict_proba(testfea)
        trainpred = gbm.predict_proba(trainfea)
        testloss = metrics.log_loss(testpred, testlabel)
...
for i in range(len(cvfold)):
    ...
    for prep in prepprocs:
        ...
        for model in models:
            perf = model(trainfea, trainlabel, testfea, testlabel, cv=False)
            #print(model)
